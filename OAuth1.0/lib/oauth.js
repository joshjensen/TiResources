...
OAuth.setProperties(OAuth, 
{
    percentEncode: function percentEncode(s) {
        ...
        var header = '';
        if (realm !== null && realm !== '') {
            header = 'OAuth realm="' + OAuth.percentEncode(realm) + '"';
        }
        var list = OAuth.getParameterList(parameters);
        list.shift();
        for (var p = 0; p < list.length; ++p) {
...
var OAuthAdapter = function(pConsumerSecret, pConsumerKey, pSignatureMethod) {
    ...
    var createMessage = function(url) {
        var message = {
            ...
            method: "POST",
            parameters: [
                ['oauth_callback', self.settings.callbackUrl], //http://tablemobile/success  // 'http://requestb.in/xs1fkhxs'
                ['oauth_version', '1.0']
            ]
        };
    ...
    this.showAuthorizeUI = function(url, callback) {
        ...
        webView.addEventListener('beforeload', function(evt) {

            if (evt.url.indexOf(self.settings.callbackUrl) !== -1) {
                var responseParams = OAuth.getParameterMap(evt.url.replace(self.settings.callbackUrl + '?', ''));

                requestToken = responseParams.oauth_token;
                oauth_verifier = responseParams.oauth_verifier;
        ...
        instance.open({model: true, modalTransitionStyle: Ti.UI.iPhone.MODAL_TRANSITION_STYLE_COVER_VERTICAL});
    };

    this.saveAccessToken = function (service) {
        accessTokenStore[service] = {
            accessToken: accessToken,
            accessTokenSecret: accessTokenSecret
        };

        Ti.App.Properties.setString("OAuth-AccessToken-" + service, JSON.stringify(accessTokenStore[service]));
    };

    this.loadAccessToken = function(service) {
        var savedData = JSON.parse(Ti.App.Properties.getString("OAuth-AccessToken-" + service));

        if (savedData !== null) {
            accessToken = savedData.accessToken;
            accessTokenSecret = savedData.accessTokenSecret;
        }
    };

    this.clearAccessToken = function(pService) {
        delete accessTokenStore[pService];
        Ti.App.Properties.setString("OAuth-AccessToken-" + service, null);
        accessToken = null;
    ...
    this.getAccessToken = function(url, callback) {
        ...
        var client = Ti.Network.createHTTPClient({
            ...
            onerror: function() {
                Ti.API.error("OAuth getAccessToken Failed");
        ...
        client.setRequestHeader("Proxy-Connection", "keep-alive");
        ...
        client.send(OAuth.formEncode(message.parameters));

    };

    this.getRequestToken = function(url, callback) {
        ...
        var client = Ti.Network.createHTTPClient({
            ...
            onerror: function() {
                Ti.API.error("OAuth getRequestToken Failed");
                Ti.API.error(this.responseText);
    ...
    this.makeRequest = function(options, callback) {
        if (accessToken === null || accessTokenSecret === null) {
            Ti.API.debug("Authorize before trying to send.");
            return;
        }

        accessor.tokenSecret = accessTokenSecret;

        var message = createMessage(options.endpointUrl);
        ...
        var client = Ti.Network.createHTTPClient({
            ...
            onerror: function() {
                Ti.API.error("OAuth makeRequest Failed");
                Ti.API.error(this.responseText);

                if (options.onError) {
                    options.onError({
                        message: "OAuth makeRequest Failed: " + this.responseText
                    });
                }
            }
        });
        ...
        if (options.requestType === 'POST') {
            client.open("POST", options.endpointUrl + options.apiPath);
        } else {
            client.open("GET", options.endpointUrl + options.apiPath);
        }
        ...
        if (options.requestType === 'POST') {
            ...
            client.send(OAuth.formEncode(options.postParams));
        }
...
exports.create = function(_settings) {
    var service = (_settings.service || "default").toLowerCase();
    var adapter = new OAuthAdapter(_settings.consumerSecret, _settings.consumerKey, (_settings.signatureMethod) ? _settings.signatureMethod : "HMAC-SHA1");
    adapter.settings = _settings;
    adapter.loadAccessToken(service);
    return {
        /**
         * @method isAuthorized
        ...
        authorize: function(callback) {
            if (!adapter.isAuthorized()) {
                adapter.getRequestToken(_settings.requestTokenUrl, function(evt) {
                    var tokenReceived = function(token) {
                        adapter.getAccessToken(_settings.accessTokenUrl, function(evt) {
                            if (evt.success) {
                                adapter.saveAccessToken(service);
                                callback();
                            }
                        });
                    };
                    ...
                    if (evt.success) {
                        adapter.showAuthorizeUI(_settings.authorizeUrl + evt.token, tokenReceived);
                    }
                });
            } else {
                callback();
        ...
        makeRequest: function(options) {
            this.authorize(function() {
                adapter.makeRequest({
                    endpointUrl: adapter.settings.endpointUrl,
